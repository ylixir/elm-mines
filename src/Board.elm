...
maxY : Board -> Int
maxY b =
    Array.length b.squares // b.width - 3
...
square x y b =
    let
        n =
            realIndex x y b
    in
    Maybe.withDefault ( Exposed, Bomb ) <| Array.get n b.squares
...
realIndex x y b =
    b.width + (y * b.width) + 1 + x


increment : Int -> Array Square -> Array Square
increment n b =
    case Maybe.withDefault ( Exposed, Bomb ) <| Array.get n b of
        ( t, Bomb ) ->
            b

        ( t, Neighbor x ) ->
            Array.set n ( t, Neighbor (x + 1) ) b
...
edge width n b =
    let
        ...
        right =
            (width - 1) == modBy width n

        top =
            width > n

        bottom =
            n + width > Array.length b
    in
...
flood width n b =
    let
        s =
            Maybe.withDefault ( Exposed, Bomb ) <| Array.get n b
    in
